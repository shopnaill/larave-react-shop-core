(function ($, window, document, undefined) {
    ...
    $.fn.imageuploadify = function (opts) {
        ...
        this.each(function () {
            ...
            let counter = 0;
            let dragbox = $(
                `<div class="imageuploadify well"><div class="imageuploadify-overlay"><i class="fa fa-picture-o"></i></div><div class="imageuploadify-images-list text-center"><i class="bx bxs-cloud-upload"></i><span class='imageuploadify-message'>قم بسحب وإسقاط ملفك (ملفاتك) هنا للتحميل </span><button type="button"class="btn btn-default">أو حدد الملف المراد تحميله</button></div></div>`
            );
                ...
                .on("submit", function (event) {
                    ...
                    for (let index = 0; index < inputs.length; ++index) {
                        if (
                            ...
                            $(inputs[index]).getAttribute("type") != "file"
                        ) {
                            formData.append(
                                inputs[index].name,
                                inputs[index].value
                    ...
                    xhr.onreadystatechange = function () {
                        if (xhr.readyState == 4 && xhr.status == 200) {
                            console.log(xhr.response);
                            product_id = xhr.response;
                            $(".progress-bar").css("width", "0%");
                            for (var i = 0; i < totalFiles.length; i++) {
                                var xhrImageUpload = new XMLHttpRequest();
                                const ImagesformData = new FormData();
                                ImagesformData.append(
                                    "image",
                                    totalFiles[i].file
                                );
                                ImagesformData.append("product_id", product_id);
                                xhrImageUpload.open(
                                    "POST",
                                    "/api/save_product_image",
                                    true
                                );
                                // upload progress event
                                xhrImageUpload.upload.addEventListener(
                                    "progress",
                                    function (e) {
                                ...
                                xhrImageUpload.addEventListener(
                                    "load",
                                    function (e) {
                                        console.log(xhrImageUpload.response);
                                    }
                                );
                                xhrImageUpload.setRequestHeader(
                                    "X-Authorization",
                                    "KkKf87BOncnOGwzjLBG8Isd87T42D1yOJJNoUMsrjmUkqSIxXmHG4XPZ07UQM0Pi"
                                );

                                xhrImageUpload.send(ImagesformData, {
                                    Accept: "application/json",
                                    "Content-Type": "application/json",
                                    "X-Authorization":
                                        "KkKf87BOncnOGwzjLBG8Isd87T42D1yOJJNoUMsrjmUkqSIxXmHG4XPZ07UQM0Pi",
                                });
                                xhrImageUpload.onreadystatechange =
                                    function () {
                                        if (
                                            xhrImageUpload.readyState == 4 &&
                                            xhrImageUpload.status == 200
                                        ) {
                                            console.log(
                                                xhrImageUpload.response
                                            );
                                            $(".progress-bar").css(
                                                "width",
                                                "0%"
